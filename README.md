# Portfolio Projects

Collection of data analytics projects by **Nikita Chistyakov** 


## 🧠 About Me

Analytics Engineer with 5+ years of experience in data across edtech & fintech. Background in building end-to-end data pipelines, data modelling, and analytics across cloud platforms.

- Building ETL/ELT pipelines (Python, SQL, Airflow)
- Data modelling with dbt on Snowflake & BigQuery
- Cloud infrastructure (GCP, AWS)
- Data visualization and reporting (Looker, Power BI)



## 🛠️ Tech Stack

- **Languages**: Python, SQL
- **Transformation**: dbt
- **Orchestration**: Airflow
- **Cloud**: GCP, AWS
- **Databases**: BigQuery, Snowflake
- **Visualization**: Looker, Power BI
- **Infrastructure**: Terraform, Git

## 📬 Contact

- GitHub: [n-chistyakov](https://github.com/n-chistyakov)
- LinkedIn: [linkedin.com/in/nikitachistyakov](https://www.linkedin.com/in/nikitachistyakov/)
- Email: chistyakov97@gmail.com


